...
class ZKDevice
{
    ...
    public function connect($timeout = 10)
    {
        ...
        $resp = $this->recv();
        if ($resp === null) {
            $this->logger->error($this->tag(), "No response to CMD_CONNECT (timeout or closed)");
            $this->closeSocket();
            return false;
        }
        ...
        if ($cmd !== self::CMD_ACK_OK) {
            $this->logger->error($this->tag(), "Handshake failed: got cmd=$cmd, expected " . self::CMD_ACK_OK);
            $this->closeSocket();
            return false;
        }
        ...
        $this->connected = true;
        $this->logger->success($this->tag(), "Connected OK (session={$this->sessionId})");
        return true;
    }
